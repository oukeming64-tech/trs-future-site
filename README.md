...
一版面向拓尔思（TRS）的开源官网焕新概念：保留现有官网的完整信息架构，用深色科技视觉、实时 WebGL 3D 数据核心、模块化内容组织和响应式交互重新表达“数据 + 智能”的品牌能力。
...
- 交互式产品矩阵：大数据、人工智能、数据安全、云与应用四类可切换
...
│   └── site/           # 可复用交互、3D 场景、导航和基础展示组件
├── data/
│   └── site-content.ts # 产品、行业、客户、公司数据和外链的唯一内容源
...
- 替换 3D 视觉：编辑 `app/components/site/DataCoreScene.tsx`
