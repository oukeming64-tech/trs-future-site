...
需要浏览器状态的组件使用 `"use client"`，其他内容在构建时预渲染为静态 HTML。
...
page.tsx composition
      ↓
static HTML + interactive client islands
```
...
- 新增联系表单：先确认第三方接收服务、数据去向和隐私文本；GitHub Pages 本身不提供后端接口。
- 新增 CMS：保持 `site-content` 的类型形状，在构建阶段建立适配层；如果需要运行时读取，必须先明确迁移托管架构。
- 新增多个路由：按业务模块建立可静态导出的路由；不要把所有逻辑塞回 `page.tsx`。
